...
// create a function that maps over the dinnerFoods array and returns a div for each food in the array
function insertDinnerFoods() {
    return dinnerFoods.map(function (food) {
        return `<div class="food">${food}</div>`
    }).join('')
}

// fetch a reference to the menu sections from the DOM
// set the innerHTML content of the menu <section> to the new function you created
document.getElementById('menu').innerHTML = insertDinnerFoods();